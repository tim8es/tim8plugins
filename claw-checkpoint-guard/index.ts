...
function readLimit(value: unknown): number {
  if (value === undefined) return DEFAULT_MAX_TOOL_CALLS;
...
function readContinuationLimit(value: unknown): number {
  if (value === undefined) return DEFAULT_MAX_AUTO_CONTINUATIONS;
...
function blockMessage(code: string, admitted: number, limit: number): string {
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    const configuredLimit = readLimit(
      rawConfig && typeof rawConfig === "object"
        ? (rawConfig as Record<string, unknown>).maxToolCallsPerRun
        : undefined,
    );
    const configuredContinuationLimit = readContinuationLimit(
      rawConfig && typeof rawConfig === "object"
        ? (rawConfig as Record<string, unknown>).maxAutoContinuations
        : undefined,
    );
    ...
    api.on("before_agent_finalize", (event, ctx) => {
      ...
      if (!guard.requestRevision(runId)) return;
    ...
    api.on("agent_end", async (event, ctx) => {
      ...
      try {
        ...
        if (sequence === undefined) {
          ...
          api.logger.warn(
            `tool-checkpoint-guard: automatic continuation limit (${configuredContinuationLimit}) reached for session`,
          );
          return;
        }
